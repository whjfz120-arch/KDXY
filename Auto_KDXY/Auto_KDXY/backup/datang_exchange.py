...
class DatangWorker(QThread):
    ...
    def reload_assets_config(self):
        ...
        # 资源路径定义
        self.crown_path = os.path.join(self.assets_dir, "crown_template.png")
        self.boss_text_path = os.path.join(self.assets_dir, "boss_text_template.png")
    ...
    def run(self):
        ...
        while self._is_running:
            ...
            while counter < 2 and self._is_running:
                ...
                # 0. 前置环节：查找 boss_text_template.png (置信度 > 0.7)，延时 1 秒
                self.log_signal.emit("🔍 [前置步骤] 正在查找 Boss 文本标记 (boss_text_template.png)...")
                boss_loc, boss_size = self.find_template_with_box(win_rect, self.boss_text_path, confidence=0.7, name="Boss文本标记")
                if not boss_loc:
                    self.log_signal.emit("❌ [前置步骤失败] 未能在超时时间内找到 boss_text_template.png，流程中断。")
                    round_success = False
                    break
                
                self.log_signal.emit("⏳ [等待] 找到 Boss 文本标记，延迟 1 秒...")
                if not self.interruptible_sleep(1.0): break

                # 1. 优先在 boss_text_template.png 的下方区域查找 crown_template.png (置信度 > 0.62)
                bx, by = boss_loc
                btw, bth = boss_size
                search_rx = 0
                search_ry = by + bth
                search_rw = win_rect['w']
                search_rh = win_rect['h'] - search_ry
                
                self.log_signal.emit(f"🔍 [步骤 1/4] 优先在 Boss 文本下方区域查找皇冠标记 (crown_template.png)...")
                # 开启 save_debug=True，保存查找皇冠的调试截图
                crown_pos = self.find_template(
                    win_rect, self.crown_path, 
                    confidence=0.62, 
                    name="皇冠标记(下方区域)", 
                    search_region=(search_rx, search_ry, search_rw, search_rh),
                    save_debug=True
                )
                
                # 如果下方区域没找到，则进行全局查找兜底
                if not crown_pos:
                    self.log_signal.emit("⚠️ [区域查找未果] 下方区域未找到皇冠，尝试全局查找 crown_template.png...")
                    crown_pos = self.wait_to_find_pos(win_rect, self.crown_path, "皇冠标记(全局兜底)", timeout=5, confidence=0.62, save_debug=True)
                
                if not crown_pos:
                    self.log_signal.emit("❌ [步骤 1/4 失败] 未能在超时时间内找到 crown_template.png，流程中断。")
                    round_success = False
                    break
                
                self.log_signal.emit(f"🖱️ [动作] 在坐标 {crown_pos} 处执行双击皇冠...")
                self.win32_double_click(crown_pos[0], crown_pos[1])
    ...
    def find_template(self, win_rect, path, confidence=0.5, name="目标", search_region=None, save_debug=False):
        ...
        if save_debug:
            try:
                ...
                debug_filename = os.path.join(self.debug_dir, f"crown_debug_{int(time.time()*1000)}.png")
                cv2.imwrite(debug_filename, debug_img)
                self.log_signal.emit(f"📷 [Debug截图] 已保存皇冠查找截图至: {debug_filename}")
            except Exception as e:
    ...
    def find_template_with_box(self, win_rect, path, confidence=0.5, name="目标"):
        if not os.path.exists(path):
            self.log_signal.emit(f"⚠️ [文件缺失] 找不到模板文件: {path}")
            return None, None
        ...
        if max_val >= confidence:
            th, tw = template.shape[:2]
            self.log_signal.emit(f"🎯 [匹配成功] [{name}] 相似度: {max_val:.3f} (达标阈值: {confidence}) -> 窗口相对位置: {max_loc}, 尺寸: ({tw}, {th})")
            return max_loc, (tw, th)
        return None, None

    def wait_to_find_pos(self, win_rect, path, name, timeout=10, confidence=0.5, save_debug=False):
        start = time.time()
        self.log_signal.emit(f"⏳ [开始寻找] 正在等待识别 [{name}] (超时限制: {timeout}秒, 目标阈值: {confidence})...")
        while time.time() - start < timeout:
            if not self._is_running:
                return None
            pos = self.find_template(win_rect, path, confidence=confidence, name=name, save_debug=save_debug)
            if pos:
                return pos
            time.sleep(0.5)
        self.log_signal.emit(f"❌ [超时警告] 寻找 [{name}] 超时 ({timeout}秒)，未检测到有效图像。")
        return None
...
class DatangExchangeModule(QWidget):
    ...
    def init_ui(self):
        ...
        layout.addWidget(QLabel("🔄 大唐物资兑换控制台 (已启用皇冠Debug截图)"))
        ...
        self.btn_start = QPushButton("启动物资兑换")
